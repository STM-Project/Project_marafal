...
static void _DrawArrayBuffRightDown2_AA(uint32_t _drawColor, uint32_t outColor, uint32_t inColor, float outRatioStart, float inRatioStart, uint32_t BkpSizeX, int direction, uint8_t *buf)
{
	...
	if(0==direction)
	{
		...
		{
			i_prev=i;
			while(i--) pLcd[k++]=drawColor;   //if(outRatioStart<1.0){} !!!!!!!!!!!!!!!!!!!!!!!!!
			//if(inRatioStart<1.0){} !!!!!!!!!!!!!!!!!!!!!!!!!
			if(0==start){  start=1;
				k-=BkpSizeX;
				if(0==outColor) _outColor=pLcd[k-i_prev];
				Set_AACoeff_Draw(i_prev,drawColor,_outColor,outRatioStart);
				for(int a=0;a<i_prev&&outRatioStart<1.0;++a){	 if(0==outColor){ if(pLcd[k-i_prev+a]!=_outColor){ _outColor=pLcd[k-i_prev+a]; Set_AACoeff_Draw(i_prev,drawColor,_outColor,outRatioStart);} } else if(pLcd[k-i_prev+a]==drawColor) break;
					pLcd[k-i_prev+a]=buff_AA[1+a]; }
				k+=BkpSizeX;
			}
...
//##################################################################################################################################################################################################################
		 if(xxxx[i].y-ABS(xxxx[i].rx) == xxxx[i+1].y  &&  xxxx[i].x+1==xxxx[i+1].x	&& (functionType==100 || functionType==RightUpDir1)){
			 ...
			 if(functionType == RightUpDir1){
				 buff[1+buff[0]++]=ABS(xxxx[i].rx);
				 _DrawArrayBuffRightUp_AA(WHITE, 0x383838, 0x383838, 1,1, LCD_X, 1, buff);
				 functionType=100;
				 buff[0]=0;
				 goto dfdfdfdfaAAAA;
			 }
...
//##################################################################################################################################################################################################################
		 if(xxxx[i].x-ABS(xxxx[i].ry) == xxxx[i+1].x  &&  xxxx[i].y+1==xxxx[i+1].y	&& (functionType==100 || functionType==LeftDownDir0)){
			 ...
			 if(functionType == LeftDownDir0){
				 buff[1+buff[0]++]=ABS(xxxx[i].ry);
				 _DrawArrayBuffLeftDown_AA(WHITE, 0x383838, 0x383838, 1,1, LCD_X, 0, buff);
				 functionType=100;
				 buff[0]=0;
				 goto dfdfdfdfaAAAA;
			 }
...
//##################################################################################################################################################################################################################
		 if(xxxx[i].x-ABS(xxxx[i].ry) == xxxx[i+1].x  &&  xxxx[i].y-1==xxxx[i+1].y	&& (functionType==100 || functionType==LeftUpDir0)){
			 ...
			 if(functionType == LeftUpDir0){
				 buff[1+buff[0]++]=ABS(xxxx[i].ry);
				 _DrawArrayBuffLeftUp_AA(WHITE, 0x383838, 0x383838, 1,1, LCD_X, 0, buff);
				 functionType=100;
				 buff[0]=0;
				 goto dfdfdfdfaAAAA;
		 ...
		 if(xxxx[i].y+ABS(xxxx[i].rx) == xxxx[i+1].y  &&  xxxx[i].x-1==xxxx[i+1].x	&& (functionType==100 || functionType==LeftDownDir1)){
			...
			functionType = LeftDownDir1;
			 ...
			 if(functionType == LeftDownDir1){
				 buff[1+buff[0]++]=ABS(xxxx[i].rx);
				 _DrawArrayBuffLeftDown_AA(WHITE, 0x383838, 0x383838, 1,1, LCD_X, 1, buff);
				 functionType=100;
				 buff[0]=0;
				 goto dfdfdfdfaAAAA;
			 }
...
//##################################################################################################################################################################################################################
		 if(xxxx[i].y-ABS(xxxx[i].rx) == xxxx[i+1].y  &&  xxxx[i].x-1==xxxx[i+1].x	&& (functionType==100 || functionType==LeftUpDir1)){
			 ...
			 if(functionType == LeftUpDir1){
				 buff[1+buff[0]++]=ABS(xxxx[i].rx);
				 _DrawArrayBuffLeftUp_AA(WHITE, 0x383838, 0x383838, 1,1, LCD_X, 1, buff);
				 functionType=100;
				 buff[0]=0;
				 goto dfdfdfdfaAAAA;
			 }
...
//_DrawArrayLeftDown_AA(WHITE, 0x383838, 0.0, LCD_X, 1, 5, 30,20,50,20,20);
	...
	uint8_t bufXX[20]={13,20,30,50,10,20,12,50,1,1,1,1,1,30,0,0,0};

	_StartDrawLine(0,LCD_X,20,210);
	_DrawArrayBuffRightDown2_AA(WHITE, 0x383838,0x383838, 1.0,1.0, LCD_X, 0, bufXX);
	_DrawArrayBuffLeftDown2_AA(WHITE, 0x383838,0x383838, 1.0,1.0, LCD_X, 1, bufXX);


	_StartDrawLine(0,LCD_X,20,230);
	_DrawArrayBuffRightDown2_AA(WHITE, 0,0, 0.0,0.0, LCD_X, 0, bufXX);
	_DrawArrayBuffLeftDown2_AA(WHITE, 0,0, 0.0,0.0, LCD_X, 1, bufXX);
