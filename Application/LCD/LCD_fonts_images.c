...
StructTxtPxlLen LCD_LIST_TXT_len(char* bufTxt, TEXT_ARRANGEMENT arangType, int fontID,int space,int constWidth, uint16_t* lenMaxStrip, uint32_t* tab,int* sizeTab,int heightWin,int spaceForUpDn)
{
	...
	{	if(*(bufTxt+i)==*_E_)
		{	switch((int)arangType){
			...
			if((NULL != tab) && ((*sizeTab) < MAX_SCREENS_FOR_LCD_LIST)){
	...
	len.inChar = CONDITION(NULL==tab,lenWholeTxt,nmbrWholeLinesInWin);
...
StructTxtPxlLen LCD_ListTxtWin(uint32_t posBuff,uint32_t BkpSizeX,uint32_t BkpSizeY,int fontID, int Xpos, int Ypos, char *txt,int seltab, int OnlyDigits, int space, uint32_t bkColor,uint32_t bkColorSel, uint32_t fontColor,uint8_t maxVal, int constWidth, uint32_t fontColorTab[], TEXT_ARRANGEMENT txtSeqRow, int spaceForUpDn)
{
	...
	StructTxtPxlLen _Txt(void){
		...
		return LCD_StrDependOnColorsWindow(posBuff,BkpSizeX,BkpSizeY,fontID,Xpos+calcPosX,SetLenTxt2Y(Ypos+nrLine*len.height,j), ptr, OnlyDigits,space,CONDITION(nrLine==seltab,bkColorSel,bkColor),color,maxVal,constWidth);
	}
